...
// Find all matching rows with *any* of the selected cluster labels
	...
	static void showPhenotypeDialog(QuPathGUI qupath, def imageData, List<Map<String, String>> cachedRows, Closure makeCoordKey, Map<String, Color> phenotypeColors) {
		...
		grid.add(viewHeatmapButton, 0, 3, 2, 1)
	...
	static javafx.scene.paint.Color getColorForValue(double value) {
		double v = Math.max(-2, Math.min(2, value))
		double ratio = (v + 2) / 4.0 // Normalize between 0 and 1

		double r = ratio <= 0.5 ? 1.0 : 1.0
		double g = 1.0 - Math.abs(ratio - 0.5) * 2
		double b = ratio >= 0.5 ? 1.0 : 1.0

		if (ratio < 0.5)
			return javafx.scene.paint.Color.color(0.5 * (1 - 2 * ratio), 0.5 * (1 - 2 * ratio), 1.0)  // Blueish
		else
			return javafx.scene.paint.Color.color(1.0, 0.5 * (2 * (ratio - 0.5)), 0.5 * (2 * (ratio - 0.5)))  // Reddish
	}

	static void viewMarkerHeatmap(List<Map<String, String>> cachedRows) {
		if (cachedRows == null || cachedRows.isEmpty()) {
			new Alert(Alert.AlertType.WARNING, "⚠️ No CSV data loaded!").show()
			return
		}

		def markerCols = ["DAPI","CD4","PD1","FOXP3","CD31","CD86","B220","CD8","aSMA","PDL1",
						  "Ki67","GZMB","FAP","CTLA4","CD11c","CD3e","CD206","F480","CD11b",
						  "CD45","Panck","Col1A1"]

		def phenotypeCols = ["Leukocytes","B_cells","Myeloid_cells","Lymphocytes",
		...
		phenotypeCols.each { pheno ->
			def rows = cachedRows.findAll { row -> (row[pheno] == "1" || row[pheno] == "1.0") }
			if (!rows.isEmpty()) {
				markerCols.each { marker ->
					def vals = rows.collect { (it[marker] ?: "0") as double }
					heatmapMatrix[pheno][marker] = vals.sum() / vals.size()
				}
				cellCounts[pheno] = rows.size()
			}
		}

		def allValues = heatmapMatrix.values().collectMany { it.values() }
		double minV = allValues.min()
		double maxV = allValues.max()
		def normalize = { double v -> (v - minV) / (maxV - minV) * 4 - 2 }

		Stage heatmapStage = new Stage()
		heatmapStage.setTitle("Phenotype vs Marker Heatmap")
		heatmapStage.setWidth(1300)
		heatmapStage.setHeight(800)

		VBox fullLayout = new VBox(10)

		// --- Color bar (Mean expression)
		HBox colorBar = new HBox(2)
		for (int i = 0; i <= 100; i++) {
			Rectangle r = new Rectangle(5, 20)
			double v = (i/100.0)*4 - 2
			r.setFill(getColorForValue(v))
			colorBar.getChildren().add(r)
		}
		Label colorLabel = new Label("Mean Expression")
		colorLabel.setStyle("-fx-font-size: 16px; -fx-font-weight: bold;")
		VBox colorBarBox = new VBox(2, colorLabel, colorBar)
		colorBarBox.setPadding(new Insets(10,0,10,150))

		fullLayout.getChildren().add(colorBarBox)

		// --- Main Heatmap + Cell counts
		GridPane grid = new GridPane()
		grid.setPadding(new Insets(10))
		grid.setHgap(2)
		grid.setVgap(2)

		markerCols.eachWithIndex { marker, col ->
			Label lbl = new Label(marker)
			lbl.setRotate(-45)
			grid.add(lbl, col + 1, 0)
		}

		phenotypeCols.eachWithIndex { pheno, row ->
			Label lbl = new Label(pheno)
			lbl.setMinWidth(150)
			grid.add(lbl, 0, row + 1)

			markerCols.eachWithIndex { marker, col ->
				double rawVal = heatmapMatrix[pheno][marker] ?: 0.0
				double normVal = normalize(rawVal)

				Rectangle rect = new Rectangle(30, 20)
				rect.setFill(getColorForValue(normVal))

				Tooltip.install(rect, new Tooltip("${pheno} - ${marker}: ${String.format('%.2f', rawVal)}"))
				grid.add(rect, col + 1, row + 1)
			}

			// Cell count bar
			int count = cellCounts.getOrDefault(pheno, 0)
			double width = Math.min(100.0, count/200.0) // adjust scaling
			Rectangle countBar = new Rectangle(width, 20)
			countBar.setFill(javafx.scene.paint.Color.GRAY)
			Tooltip.install(countBar, new Tooltip("${pheno}: ${count} cells"))
			grid.add(countBar, markerCols.size() + 1, row + 1)

			Label countLabel = new Label("${count}")
			grid.add(countLabel, markerCols.size() + 2, row + 1)
		}

		ScrollPane scrollPane = new ScrollPane(grid)
		fullLayout.getChildren().add(scrollPane)

		Scene scene = new Scene(fullLayout)
		heatmapStage.setScene(scene)
		heatmapStage.show()
	}
