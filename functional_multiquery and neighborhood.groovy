...
// Multi-Query Search with conditional neighborhood binding for optimal speed
	private static void runMultiQuerySearch(QuPathGUI qupath) {
		...
		btnRun.setOnAction {
			...
			def allVectors = allCells.collectEntries { c -> [c, extractionMethod(c, allCellsMap, radiusPixels)] }
			def similarSets = selectedCells.collect { center ->
				def targetVec = allVectors[center]
				def distances = allCells.findAll { it != center }.collect { c ->
					[c, new EuclideanDistance().compute(targetVec, allVectors[c])]
				}.sort { it[1] }
				distances.take(limit).collect { it[0] } as Set
			}
			...
			if (cbUnion.isSelected()) finalResults = similarSets.flatten()
			else if (cbIntersection.isSelected()) finalResults = similarSets.inject(similarSets[0]) { acc, s -> acc.intersect(s) }.toList()
			else if (cbSubtract.isSelected()) finalResults = similarSets[0] - similarSets[1..-1].flatten()
			else if (cbContrastive.isSelected()) finalResults = similarSets[0] - similarSets[1]

			def multiQueryClass = PathClass.fromString("Multi-Query-Search")
